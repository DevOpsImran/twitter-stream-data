...
*/
 mongoose.connect("mongodb://localhost:27017/public_stream_db");
...
var date_at=DD+"/"+MM+"/"+YY+"-"+H+":"+M+":"+S+":"+MS;
       //console.log(">>>>"+date_at);
        ...
        var twit_entity=tweet.entities;
        var twit_loc=tweet.user.location;
        var sample=tweet.user.geo_enabled;
        
        var followers_count = tweet.user.followers_count;
        var friends_count = tweet.user.friends_count;
        var favourites_count = tweet.user.favourites_count;
//console.log(followers_count+","+friends_count+","+favourites_count);
        //console.log(twit_user_screen_name+" >>> "+sample+" >>"+twit_loc);

       
      //console.log(twit_createdAt);
...
// ###
        ...
        var geo_null=tweet.geo;
           ...
           console.log(geo_coordinates);
           console.log(geo_coordinates[0] +"and"+geo_coordinates[1]);
            }
...
/* Code for Twitter Entities */
        ...
        var entiti_hashtag_set=[];
           ...
           // console.log("HASH_TAG  "+entiti_hashtag_set)
            }
            
            var user_mention;  
            var entiti_user_mentions_count=tweet.entities.user_mentions.length;
            var entiti_user_mentions_set=[];
            if(entiti_user_mentions_count > 0)
            {
            	//console.log(entiti_user_mentions_count);
                for(i=0;i<entiti_user_mentions_count;i++)
                {
                //console.log(tweet.entities.hashtags[i].text);
                user_mention=tweet.entities.user_mentions[i].screen_name;//.length;
                //console.log(user_mention)
                entiti_user_mentions_set.push(user_mention);
                }
             //   console.log("MENTIONS  "+entiti_user_mentions_set);
